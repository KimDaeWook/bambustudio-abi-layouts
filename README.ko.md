...
마지막 job은 두 결과의 version과 정확한 upstream source commit이 같을 때만 결합합니다. 이후 `abi-layouts/<version>/macos-arm64.json`을 생성하고 artifact로 업로드한 뒤 저장소에 commit합니다. Target JSON에는 런타임이 사용하는 `symbols`, `event_bindings`, `layouts` 전체가 들어가고, `manifest.json`에는 provenance와 hash가 들어갑니다. 파일 하나가 target architecture 하나만 표현하므로 UUID와 address는 architecture map이 아닌 scalar입니다. Event filter 함수는 `symbols`에 들어가고 kind/reason이 필요한 event global만 `event_bindings`에 남습니다. 각 layout 값에는 C++ type과 실제 member 또는 합성 member path가 함께 기록되어 의미를 바로 추적할 수 있습니다. Layout, function, 필수 event, architecture, UUID 또는 commit 일치 중 하나라도 빠지면 hard failure입니다.
