...
마지막 job은 두 결과의 version과 정확한 upstream source commit이 같을 때만 결합합니다. 이후 `abi-layouts/<version>/macos-arm64.json`을 생성하고 artifact로 업로드한 뒤 저장소에 commit합니다. Target JSON에는 `symbols`, `event_bindings`, `layouts`, `vtables`가 들어가고, `manifest.json`에는 provenance와 hash가 들어갑니다. 파일 하나가 target architecture 하나만 표현하므로 UUID와 address는 architecture map이 아닌 scalar입니다. Event filter 함수는 `symbols`에 들어가고 kind/reason이 필요한 event global만 `event_bindings`에 남습니다. Layout은 실제 C++ type을 key로 사용하며 직접적인 `members`, `bases`, `size`, `alignment` 사실만 포함합니다. 확장 기능별 group, 임의 alias, 미리 합산한 접근 path 또는 UI metadata는 포함하지 않고 소비자가 필요한 직접 offset을 조합합니다. Record, vtable, function, 필수 event, architecture, UUID 또는 commit 일치 중 하나라도 빠지면 hard failure입니다.

각 버전은 `abi-layouts/<version>/requirements.json`을 소유합니다. 이 파일은 실제 C++ 이름을 key로 사용합니다. Record type에는 요청 member/base가, 전체 C++ 함수 또는 global 이름에는 검토된 mangled symbol이, vtable type에는 요청 method signature가 연결됩니다. Runtime logical method 이름이나 event 분류는 포함하지 않습니다. 두 extractor 모두 이 파일을 읽으며 검증 개수도 여기서 계산합니다. 요청 버전에 requirements가 아직 없으면 resolver가 숫자 기준으로 가장 가까운 이전 버전을 복사하고 `inherited_from`을 기록하며, complete workflow가 결과와 함께 새 버전 전용 파일을 commit합니다. 따라서 하나의 global catalog를 영구적으로 유효하다고 암묵적으로 취급하지 않고 상속 관계를 명시적으로 검토할 수 있습니다.
